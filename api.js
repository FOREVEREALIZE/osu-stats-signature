...
export const getUser = async (username, playmode = 'std', includeTopPlays = false, includeSkills = false, lazer = false) => {
	if (username == '@example') {
	...
	try {
		response = await got({
			method: 'get',
			url: `https://${lazer ? 'lazer' : 'osu'}.ppy.sh/users/${username}/${playmodes[playmode]}`,
		});	
	} catch (error) {
	...
	if (includeTopPlays) {
		response = await got({
			method: 'get',
			url: `https://${lazer ? 'lazer' : 'osu'}.ppy.sh/users/${data.user.id}/extra-pages/top_ranks?mode=${playmodes[playmode]}`,
		});
